// Redone simpleCUBLASDouble.cpp, comparing a matrix multiplication on the cpu, gsl-blas and cublas
...
static void simple_sgemm(int n, double alpha, const double *A, const double *B,double beta, double *C) {
    int i;
    int j;
    int k;
...
int main(int argc, char **argv) {
	...
	A_cpu = (double *)malloc(n2 * sizeof(double));
	if (A_cpu == 0) {
		fprintf(stderr, "!!!! host memory allocation error (A)\n");
		return EXIT_FAILURE;
	}

	B_cpu = (double *)malloc(n2 * sizeof(double));
	if (B_cpu == 0) {
	...
	C_ref_cpu = (double *)malloc(n2 * sizeof(double));
	if (C_ref_cpu == 0) {
		fprintf(stderr, "!!!! host memory allocation error (C)\n");
		return EXIT_FAILURE;
	}
	C_blas_cpu = (double *)malloc(n2 * sizeof(double));
	if (C_blas_cpu == 0) {
		fprintf(stderr, "!!!! host memory allocation error (C)\n");
		return EXIT_FAILURE;
	}
	...
	for (ui = 0; ui < N; ui++) {
		for (uj = 0; uj < N; uj++) {
			A_cpu[uj*N+ui] = rand() / (double)RAND_MAX;
			B_cpu[uj*N+ui] = rand() / (double)RAND_MAX;
			//C_cpu[ui] = rand() / (double)RAND_MAX;
			//A_cpu[uj*N+ui] = (double)(ui)/(double)(N);
			//B_cpu[uj*N+ui] = 1.0;
			C_cpu[uj*N+ui] = 0.0;
	...
	status = cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, N, N, N, &alpha, B_gpu, N, A_gpu, N, &beta, C_gpu, N);
	...
	C_cpu = (double *)malloc(n2 * sizeof(C_cpu[0]));
	...
	printf("Total time with host: %f, with gslblas: %f, with cuBlas: %f, speedup to host:%f, speedup to blas:%f\n",
		...
		(double)(hostElapsedTime/cuBlasElapsedTime),(double)(blasElapsedTime/cuBlasElapsedTime));
