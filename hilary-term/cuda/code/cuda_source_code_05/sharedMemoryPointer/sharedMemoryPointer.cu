...
// the same
__device__ void scanLocalTheadInformation (int *pointerToSharedMemory) {
	*(pointerToSharedMemory)= threadIdx.x;
}
...
__global__ void scanTheadInformationGPU(int *threadIdsGPU,int *threadOtherIdsGPU,int *blockIdsGPU,int Ntot) {
	...
	scanLocalTheadInformation(&sharedMemoryThread[threadIdx.x]);
