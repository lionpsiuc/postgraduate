...
//__device__ __constant__ float   MatrixCConstantGPU[100000000000];
...
__device__ void assignData(float *basicFloatArrayGPU, int *nConst, int *mConst) {
	...
	basicFloatArrayGPU[idy+idx*(*mConst)]=-10.1;
}
...
__global__ void multiplyMatricesGPU(float *basicFloatArrayGPU) {
	...
	printf ("multiplyMatricesGPU::threadIdx[%d,%d] (%d,%d)\n",idx,idy,myX,myY);
	
	if (myX<DIM_X) {
	   if (myY<DIM_Y){
		basicFloatArrayGPU[idy+idx*MCONST]=0.0f;
		for (i=0;i<MCONST;i++) {
			printf ("multiplyMatricesGPU::threadIdx[%d,%d] i=(%d) MCONST=(%d)\n",idx,idy,i,MCONST);
			printf ("multiplyMatricesGPU::threadIdx[%d,%d] MatrixAConstantGPU=(%f)\n",idx,idy,MatrixAConstantGPU[myX][i]);
			basicFloatArrayGPU[idy+idx*MCONST]+=MatrixAConstantGPU[myX][i]*MatrixBConstantGPU[i][myY];


			assignData(basicFloatArrayGPU,&NCONST,&MCONST);
		}
	   }	
	}
...
int main() {
	...
	cudaMemcpyToSymbol(MCONST		,&M			,    sizeof(int)		);
...
//	cudaMemcpyToSymbol("MatrixBConstantGPU"	,MatrixB1d		,    M*N*sizeof(float)		);
	...
	checkCUDAError ("multiplyMatricesGPU finsihed");
