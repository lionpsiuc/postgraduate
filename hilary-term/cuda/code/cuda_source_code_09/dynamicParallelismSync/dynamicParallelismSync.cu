//=============================================================================================
// Name        		: dynamicParallelismSync.cu
// Author      		: Jose Refojo
// Version     		:	19-03-14
// Creation date	:	19-03-14
// Copyright		: Copyright belongs to Trinity Centre for High Performance Computing
// Description		: This program will show how to synchronization child threads with parent ones.
//=============================================================================================
...
__global__ void add_arrays_gpu( float *in1, float *in2, float *out, int Ntot) {
	...
	if ( idx <Ntot ) {
		...
		if (idx==0) {
			child_kernel<<<16, 1>>>(in1,in2,out,Ntot);
			// If we do this, the child kernel will be executed AFTER:
			//__syncthreads();
			out[idx]=1;
		}
		if (idx==1) {
			child_kernel<<<16, 1>>>(in1,in2,out,Ntot);
			// If we do this, the child kernel will be exectuted BEFORE:
