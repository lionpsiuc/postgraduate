...
int main(void) {
	...
	float *a_device_0; // pointer to device 0 memory
	float *a_device_1; // pointer to device 1 memory
...
#error "This CUDART version does not support mapped memory!\n"
	...
	cudaSetDevice(0);
	cudaMalloc ((void **) &a_device_0, sizeof(float)*N);
	checkCUDAError("cudaMalloc a_device_0");
	// We will allocate memory in the second available device
	cudaSetDevice(1);
	cudaMalloc ((void **) &a_device_1, sizeof(float)*N);
	checkCUDAError("cudaMalloc a_device_1");
	...
	// Copy data from host memory to device 0 memory
	cudaSetDevice(0);
	cudaMemcpy(a_device_0, a_host, sizeof(float)*N, cudaMemcpyHostToDevice);
	checkCUDAError("cudaMemcpyHostToDevice");

	// increment one in device 0
	incrementArrayOnDevice <<< nBlocks, blockSize >>> (a_device_0, N);
	checkCUDAError("incrementArrayOnDevice in device 0");

	// Set the second device
	cudaSetDevice(1);
	// Copy 
	cudaMemcpyPeer(a_device_1, 1, a_device_0, 0, size);
	// Then increment one in device 1
	incrementArrayOnDevice <<< nBlocks, blockSize >>> (a_device_1, N);
	...
	cudaMemcpy(a_host, a_device_1, sizeof(float)*N, cudaMemcpyDeviceToHost);
	...
	// check results
	printf("If the memory copy worked correctly, all these values should be the same:\n");
	for (i=0; i<N; i++) {
