...
__global__ void computeRowSumsKernel(float* matrix_flat, float* rowSums, int n, int m) {
...
__global__ void computeColumnSumsKernel(float* matrix_flat, float* colSums, int n, int m) {
...
extern "C" float* computeRowSumsGPU(float** matrix, int n, int m, int threads_per_block) {
    ...
    cudaMemcpy(d_matrix, matrix_flat, n * m * sizeof(float), cudaMemcpyHostToDevice);
    cudaCheckError();
    ...
    // Launch kernel
    computeRowSumsKernel<<<numBlocks, threads_per_block>>>(d_matrix, d_rowSums, n, m);
    cudaCheckError();
...
extern "C" float* computeColumnSumsGPU(float** matrix, int n, int m, int threads_per_block) {
    // Flatten the matrix for GPU
    float* matrix_flat = flattenMatrix(matrix, n, m);
    ...
    // Launch kernel
    computeColumnSumsKernel<<<numBlocks, threads_per_block>>>(d_matrix, d_colSums, n, m);
    cudaCheckError();
