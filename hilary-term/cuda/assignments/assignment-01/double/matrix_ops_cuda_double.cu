...
__global__ void computeRowSumsKernelDouble(double* matrix_flat, double* rowSums, int n, int m) {
...
__global__ void computeColumnSumsKernelDouble(double* matrix_flat, double* colSums, int n, int m) {
...
extern "C" double* computeRowSumsGPUDouble(double** matrix, int n, int m, int threads_per_block) {
    ...
    cudaMemcpy(d_matrix, matrix_flat, n * m * sizeof(double), cudaMemcpyHostToDevice);
    cudaCheckError();
    ...
    // Launch kernel
    computeRowSumsKernelDouble<<<numBlocks, threads_per_block>>>(d_matrix, d_rowSums, n, m);
    cudaCheckError();
...
extern "C" double* computeColumnSumsGPUDouble(double** matrix, int n, int m, int threads_per_block) {
    // Flatten the matrix for GPU
    double* matrix_flat = flattenMatrixDouble(matrix, n, m);
    ...
    // Launch kernel
    computeColumnSumsKernelDouble<<<numBlocks, threads_per_block>>>(d_matrix, d_colSums, n, m);
    cudaCheckError();
...
extern "C" double reduceGPUDouble(double* vector, int size, int threads_per_block) {
    // Allocate device memory
    ...
    reduceKernelDouble<<<numBlocks, threads_per_block, threads_per_block * sizeof(double)>>>(
    ...
    if (numBlocks > 1) {
        ...
        cudaMemcpy(partial_sums, d_partial_sums, numBlocks * sizeof(double), cudaMemcpyDeviceToHost);
        cudaCheckError();
