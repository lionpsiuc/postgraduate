...
void printUsage(char *programName) {
  ...
  printf(
      "  -b <size>      Number of threads per block for CUDA (default: 256)\n");
  printf("  -r             Use current time as random seed (default: fixed "
         "seed 1234567)\n");
  printf("  -t             Display timing information\n");
  printf(
      "  -o <filename>  Write results to file (default: results_double.csv)\n");
...
int main(int argc, char *argv[]) {
  ...
  clock_gettime(CLOCK_MONOTONIC, &start);
  double *colSumsGPU =
      computeColumnSumsGPUDouble(matrix, n, m, threads_per_block);
  clock_gettime(CLOCK_MONOTONIC, &end);
  gpu_col_time =
      (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;
