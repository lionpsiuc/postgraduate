...
void writeResults(const char* filename, int n, int m, int threads_per_block, 
                  ...
                  float cpu_row_sum, float cpu_col_sum, float gpu_row_sum, float gpu_col_sum) {
    ...
    if (size == 0) {
        fprintf(file, "n,m,threads_per_block,"
    ...
    // Write results
    fprintf(file, "%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.2f,%.2f,%.2f,%.2f,%.8f,%.8f\n",
