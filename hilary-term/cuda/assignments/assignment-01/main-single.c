 ...
 * @file main-single.c
 *
 * @brief Main programme for matrix operations benchmark in single precision.
 *
...
int main(int argc, char *argv[]) {
  ...
  float cpu_row_time = 0.0f, cpu_col_time = 0.0f;
  float cpu_reduce_row_time = 0.0f, cpu_reduce_col_time = 0.0f;
  float gpu_row_time = 0.0f, gpu_col_time = 0.0f;
  float gpu_reduce_row_time = 0.0f, gpu_reduce_col_time = 0.0f;
  ...
  rowSums = computeRowSums(matrix, n, m);
  ...
  // Column sums
  clock_gettime(CLOCK_MONOTONIC, &start);
  colSums = computeColumnSums(matrix, n, m);
  clock_gettime(CLOCK_MONOTONIC, &end);
  cpu_col_time =
      (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;
  ...
  float rowSumGPU = 0.0f;
  float colSumGPU = 0.0f;
  float *rowSumsGPU = NULL;
  float *colSumsGPU = NULL;
  ...
  if (!cpuOnly) {
    ...
    // Row sums
    clock_gettime(CLOCK_MONOTONIC, &start);
    rowSumsGPU = computeRowSumsGPU(matrix, n, m, threads_per_block);
    clock_gettime(CLOCK_MONOTONIC, &end);
    gpu_row_time =
        (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;
    ...
    // Row reduction
    clock_gettime(CLOCK_MONOTONIC, &start);
    rowSumGPU = reduceGPU(rowSumsGPU, n, threads_per_block);
    clock_gettime(CLOCK_MONOTONIC, &end);
    gpu_reduce_row_time =
        (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;
    ...
    // Column sums
    clock_gettime(CLOCK_MONOTONIC, &start);
    colSumsGPU = computeColumnSumsGPU(matrix, n, m, threads_per_block);
    clock_gettime(CLOCK_MONOTONIC, &end);
    gpu_col_time =
        (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;
    ...
    // Column reduction
    clock_gettime(CLOCK_MONOTONIC, &start);
    colSumGPU = reduceGPU(colSumsGPU, m, threads_per_block);
    clock_gettime(CLOCK_MONOTONIC, &end);
    gpu_reduce_col_time =
        (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;
  ...
  if (!cpuOnly && writeToFile) {
    writeResults(outputFilename, n, m, threads_per_block, cpu_row_time,
                 cpu_col_time, cpu_reduce_row_time, cpu_reduce_col_time,
                 gpu_row_time, gpu_col_time, gpu_reduce_row_time,
                 gpu_reduce_col_time, rowSum, colSum, rowSumGPU, colSumGPU);
  }
