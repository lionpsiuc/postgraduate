/**
 * @file assignment4.cc
 * @brief Assignment 4 for 5614 C++ programming
...
int main() {
  try {
    ...
    /*
     *                // Test 0: This should be caught by Number Concept at
     *compile time if uncommented HPC::Matrix <std::string> V2 {std::string
     *{"vector.txt"}};
     *
     * 		  // Test 1:  Should throw error as file does not exist
     *		  HPC::Matrix <double> X1 {std::string {"matrix_10x10.txt"}};
     *
     *                // Test 2: This should throw an out-of-range error for
     *operator() HPC::Matrix <double> E {10,10}; E(99,1) = 1.0;
     *
     *                // Test 3: Should throw error due to incompatible
     *dimensions std::cout << "M1+M2:\n" << M1+M2  << std::endl;
     *
     *                // Test 4: Should throw error due to incompatible
     *dimensions std::cout << "V1xM1:\n" << V1*M1 << '\n';
     *
    ...
    // Find inverse of M1
    HPC::Matrix<double> I{M1.get_num_rows(),
                          M1.get_num_cols()}; // Construct a rows x cols Matrix
