 ...
 *  	Modified from
 * https://isocpp.org/wiki/faq/operator-overloading#matrix-subscript-op Store
 * the matrix data as a single C-style array Students need to write function
 * definitions for all the included member functions (except the deleted
 * constructor)
 *
...
public:
  ...
  Matrix(std::size_t const rows,
         std::size_t const cols); // Overloaded Constructor
  Matrix(
      std::string const file); // Overloaded Constructor. Read data from file.
  T &operator()(std::size_t const row_num,
                std::size_t const col_num); // non-const version which can be
                                            // used to modify objects
  T operator()(std::size_t const row_num, std::size_t const col_num)
      const; // Const version to be called on const objects
 ...
 * 		Don't need to check for negative values as size_t will be
 * unsigned.
 *
