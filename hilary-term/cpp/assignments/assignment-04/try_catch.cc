/**
 * @file try_catch.cc
 * @brief  Short program to implement try-catch blocks to process some
 * exceptions.
 * @author R. Morrin
 * @version 1.0
 * @date 2024-03-09
...
void throw_fun(const double in) {
  ...
  if (in < 0.75) {
    ...
    [[maybe_unused]] double *t{
        new double[s]}; // The "[[maybe_unused]]" attribute just prevents
                        // compiler warnings for not using this.
  }
