/**
 * @file vector_swap_move.cc
 * @brief 5614 L6 2021 examples.
 * @author R. Morrin
 * @version 2.0
 * @date 2021-02-16
 */
...
namespace HPC
{
	...
	public:
	    ...
	    Vector incremented_copy(const Vector in); // You can ignore for L6
    ...
    void swap_move(Vector &A, Vector &B){
	std::cout << "Swapping using move\n";
	Vector temp {std::move(A)};
	A = std::move(B);
	B = std::move(temp);
    }
} /* HPC */ 
